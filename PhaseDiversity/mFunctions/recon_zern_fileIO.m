...
%   penalChioce: penalty term to image, 
%       1 for L2 norm of image and 2 for L2 norm of gradient
% Modification: Feb. 17, 2022
%   parameters to match new microscope setup: 
%       pixelsize: 0.08um; rotation angle: 90; and flipping setting
...
lambda = 0.532; % um
NA = 1.2;
rotAng = 90; % image rotation angle, (unit: degree);
...
coeffsInitialLv = coeffsLv(1,pStart:pEnd);
% phase diversity:
...
coeffsOutLv = coeffsOut(pStart:pEnd);
...
if(flagShowRecon == 1)
    ...
    figure; % plot Zernike coefficients as MATLAB convention
    ...
    plot(pIn, coeffsInitialLv,pIn,coeffsOutLv,'LineWidth',2);
    legend( 'input','estimated');
    xlabel('Zernike Index');
    ylabel('Coeff Value');
    set(gca,'FontSize', 14);
    title('Zernike Modes (LabVIEW)');
    savefig([folderOut 'coeff.fig']);
end
cTime = toc;
