function [cEstimate, imgEstimate, J, itTotal, cInter, ws] = zernretrieve_loop_camera2P(imgDs, ...
    r0, theta0, idx, p, waveFront_deltas, c0, zernPolynomials, Rc, ...
    penalChioce, gamma, alpha, itLimit, stopChoice, tolValue, GPUflag, nFlag, ...
    r0Exc, theta0Exc, idxExc, waveFront_deltasExc)
...
rExc = r0Exc(idxExc); % convert 2D matrix to vector with selected elements
thetaExc = theta0Exc(idxExc); % convert 2D matrix to vector with selected elements
...
ws = zeros(itLimit+1, 2, 'single'); % w factor of 2P excitation (initial + all iterations)
...
if(GPUflag==1)
    ...
    pupilMaskExc = zeros(Sx, Sy, 'single', 'gpuArray');
    waveFrontExc = zeros(Sx,Sy, 'single', 'gpuArray');
    ...
    pupilMaskExc = zeros(Sx, Sy, 'single');
    waveFrontExc = zeros(Sx,Sy, 'single');
...
waveFrontExc(idxExc) = create_wavefront(p, coeffs0, rExc, thetaExc, nFlag);
phiExc = waveFrontExc; %phase
HkExc = pupilMaskExc.*exp(1i*phiExc); % pupil function
hkExc = ifft2(HkExc); % prh
skExc = abs(hkExc).^4; % PSF
wFactor0 = mean(skExc(:));
...
for i = 1:itLimit+1
    ...
    waveFrontExc(idxExc) = create_wavefront(p, c, rExc, thetaExc, nFlag);
    for k = 1:imgNum
        ...
        % excitation part        
        phiExc = waveFrontExc+waveFront_deltasExc(:,:,k); %phase
        HkExc = pupilMaskExc.*exp(1i*phiExc); % pupil function
        hkExc = ifft2(HkExc); % prh
        skExc = abs(hkExc).^4; % PSF
        wFactor = mean(skExc(:));
        wFactor =  wFactor/wFactor0;
        ws(i,k) = wFactor;
