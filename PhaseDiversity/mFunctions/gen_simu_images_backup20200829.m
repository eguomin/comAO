...
if(~strcmp(nType, 'none'))
    ...
    % singal defined as the intensity average of the pixels > threshold.
    threPerc = 0.1; % singal defined as the intensity average of the pixels > threshold.
    threValue = mean(img0(:)) * threPerc;
    mask = img0>threValue;
    I_mask = img0 .* mask;
    S0 = sum(I_mask(:))/sum(mask(:));
    
    % image scaling ratio
    scaleR = S/S0;
else
    % scale image to maximam value of 8192;
    scaleR = 8192/max(img0(:));
end
...
%         G = 10;
%         img = addnoiseSNR(imgBlur, nType, SNR, G, 3);
    end
