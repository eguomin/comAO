function [cEstimate, imgEstimate, rePar] = recon_zern_camera2P(imgs, p, c_delta0, gamma,...
    alpha, itLimit, zernSteps, pixelSize, lambdaExc, lambdaDet, NA, GPUflag, penalChioce,rotAng)
...
%   lambdaExc: excitation wavelength, unit: um
%   lambdaDet: detection wavelength, unit: um
...
length2phase = 2*pi/lambdaDet;
c_delta = length2phase * c_delta0;

% Define the pupil coordinates (Polar coordinate system) 
[r0, theta0, idx] = def_pupilcoor(Sx, pixelSize, lambdaDet, NA);
...
% Excitation part
% Zernike coefficients: convert lengh unit(um) to phase unit(pi)
length2phaseExc = 2*pi/lambdaExc;
c_deltaExc = length2phaseExc * c_delta0;

% Define the pupil coordinates (Polar coordinate system) 
[r0Exc, theta0Exc, idxExc] = def_pupilcoor(Sx, pixelSize, lambdaExc, NA);
theta0Exc = theta0Exc + deg2rad(rotAng);

% pre-calculation
[~, ~, ~, waveFront_deltasExc] = zernretrieve_pre(imgs, ...
    r0Exc, theta0Exc, idxExc, p, c_deltaExc, nFlag);
...
if(GPUflag == 1)
    ...
    waveFront_deltasExc = gpuArray(waveFront_deltasExc);
end
for iStep = 1:stepNum
    ...
    [cEstimate, imgEstimate, J, itTotal, cInter, ws] = zernretrieve_loop_camera2P(imgDs,...
