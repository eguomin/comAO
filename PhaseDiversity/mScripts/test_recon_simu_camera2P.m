% A simple example: test 2P phase diveristy based on synthetic images.
...
gamma = 1e-8; % gamma range: 1e-14 ~ 1e-6
alpha = 0;
penalChioce = 1;
...
[Sx, ~] = size(img0);
fileFolderOut = '..\DataForTest\Simu\TestResults_camera2P_2\';
...
[imgs, PSFs, waveFronts, PSF_Exc2P_norms] = gen_simu_images_camera2P(img0, pIn, coeffs_all, ...
...
[cEstimate, imgEstimate, rePar] = recon_zern_camera2P(imgs, pIn, coeffs_delta, ...
    gamma, alpha, itLimit, zernSteps, pixelSize, lambdaExc, lambdaDet, NA, flagGPU, penalChioce);
cTime2 = toc(tStart);
disp(['... ... time cost: ', num2str(cTime2-cTime1)]);

% calculate RMS and PV error
[~, ~, staParaInitial, ~] = coeffs2wavefront(pIn,coeffsInitial,Sx,...
    pixelSize, lambdaDet, NA, 0);
